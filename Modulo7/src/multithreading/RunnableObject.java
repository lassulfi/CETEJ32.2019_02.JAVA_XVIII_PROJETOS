...
public class RunnableObject implements Runnable {
    ...
    private JLabel threadLabel; // JLabel for output
    private JLabel countdownJLabel; // JLabel for countdown
    ...
    public RunnableObject(Lock theLock, JLabel label) {
        ...
        threadLabel = label; // store JLabel for outputting character
    } // end RunnableObject constructor
    
    public RunnableObject(Lock theLock, JLabel threadLabel, JLabel countdownJLabel, Buffer shared) {
    	this.lockObject = theLock;
    	this.suspend = lockObject.newCondition();
    	this.threadLabel = threadLabel;
    	this.countdownJLabel = countdownJLabel;
        ...
        while (true) // infinite loop; will be terminated from outside
        {
            ...
            Runnable objr = new Runnable() {
                        ...
                        public void run() {
                            ...
                            threadLabel.setText(threadName + ": " + displayChar);
    ...
    public void toggle() {
    	int time = Integer.valueOf(countdownJLabel.getText()); // Gets the current time in the label
    	
        suspended = !suspended; // toggle boolean controlling state

        // change label color on suspend/resume
        threadLabel.setBackground(suspended ? Color.RED : Color.GREEN);
        ...
        try {
            ...
            {
            	this.sharedLocation.stopCountdown(time);
                suspend.signal(); // resume thread
            } else { //thread stoped. Starts countdwon            	
            	this.sharedLocation.startCountdown(time);
            }// end if
        } // end try
        finally {
